...
const (
	...
	// can read it back. /tmp rather than /workspace: a spill is scratch for one
	// conversation, and /tmp is an emptyDir that dies with the pod, whereas
	// /workspace is a PVC shared by every run in a workspace session — output
	// parked there would outlive the run that produced it. Both are roots
	// read_file will open, and the directory is only created when elision fires.
	defaultElisionSpillDir = "/tmp/sympozium/elided"
)
...
type contextPolicy struct {
	...
	// disables spilling, and elision falls back to a bare placeholder. It is
	// scoped to the run (see runScopedSpillDir), so runs sharing a directory
...
func loadContextPolicy() contextPolicy {
	...
	cp.SpillDir = runScopedSpillDir(cp.SpillDir, getEnv("AGENT_RUN_ID", ""))
...
// runScopedSpillDir gives each run its own subdirectory under base, so a spill
// directory that outlives a single run cannot have the next run's 0000.txt
// overwrite the previous run's — the filenames are ledger positions, which
// every run reuses from zero. It matters most when the directory is pointed at
// /workspace, whose PVC is shared by every run in a workspace session.
//
// The joined path is checked to still be inside base rather than assumed to be:
// runID comes from the environment, and AGENT_RUN_ID being a Kubernetes object
// name today is a property of one caller, not of the path we are about to
// write to. A segment that would climb out drops the scoping instead.
func runScopedSpillDir(base, runID string) string {
	if base == "" || runID == "" {
		return base
	}
	clean := filepath.Clean(base)
	scoped := filepath.Clean(filepath.Join(clean, runID))
	if scoped == clean || !underAnyPrefix(scoped, []string{clean}) {
		return base
	}
	return scoped
}
