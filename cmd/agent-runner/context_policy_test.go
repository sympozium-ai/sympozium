...
// tempSpillDir returns a disposable spill directory under /tmp. It has to be
// /tmp for the same reason writeReadFileFixture does: read_file only opens
// paths under its allowlisted roots, and t.TempDir() lands outside them on
// macOS — which would silently turn spilling off and test nothing.
...
// The default has to be somewhere the spill dies with the run that wrote it.
// /workspace does not qualify: a workspace session shares one PVC across runs.
func TestLoadContextPolicy_SpillDirDefaultIsEphemeral(t *testing.T) {
	if strings.HasPrefix(defaultElisionSpillDir, "/workspace") {
		t.Errorf("default spill dir %q outlives the run on a session PVC", defaultElisionSpillDir)
	}
	if !underAnyPrefix(defaultElisionSpillDir, readableRoots) {
		t.Errorf("default spill dir %q is outside the roots read_file can open", defaultElisionSpillDir)
	}
}

// Spill filenames are ledger positions, which every run reuses from zero, so a
// directory shared between runs needs the run to be part of the path.
func TestLoadContextPolicy_SpillDirIsScopedPerRun(t *testing.T) {
	t.Setenv("CONTEXT_ELISION_SPILL_DIR", "/workspace/.sympozium/elided")

	t.Setenv("AGENT_RUN_ID", "run-a")
	a := loadContextPolicy().SpillDir
	t.Setenv("AGENT_RUN_ID", "run-b")
	b := loadContextPolicy().SpillDir

	if a == b {
		t.Fatalf("two runs share spill dir %q — 0000.txt would overwrite", a)
	}
	if want := "/workspace/.sympozium/elided/run-a"; a != want {
		t.Errorf("SpillDir = %q, want %q", a, want)
	}
	// Still readable, or the stub would point at a file the agent cannot open.
	if reason := spillUnavailable(a, []ToolDef{{Name: ToolReadFile}}); reason != "" {
		t.Errorf("run-scoped dir should stay spillable, got %q", reason)
	}

	// The opt-out stays an opt-out — scoping must not resurrect a disabled spill.
	t.Setenv("CONTEXT_ELISION_SPILL_DIR", "off")
	if got := loadContextPolicy().SpillDir; got != "" {
		t.Errorf("SpillDir = %q, want \"\" — scoping must not undo the opt-out", got)
	}
}

// runID reaches the policy through the environment, so the join is verified
// rather than trusted: a segment that climbs out of the base drops the scoping
// instead of relocating the spill.
func TestRunScopedSpillDir_CannotEscapeBase(t *testing.T) {
	const base = "/tmp/sympozium/elided"
	for _, runID := range []string{"..", "../..", "../../etc", "./..", "."} {
		if got := runScopedSpillDir(base, runID); got != base {
			t.Errorf("runScopedSpillDir(%q, %q) = %q, want the unscoped base", base, runID, got)
		}
	}
	if got := runScopedSpillDir("", "run-a"); got != "" {
		t.Errorf("disabled spilling must stay disabled, got %q", got)
	}
	if got := runScopedSpillDir(base, ""); got != base {
		t.Errorf("an unset run ID must leave the base alone, got %q", got)
	}
}
...
func TestDirSpiller_PathIsDerivedOnlyFromIndex(t *testing.T) {
	const dir = "/tmp/sympozium/elided/run-xyz"
...
func TestRunAgentLoop_ElidesAtHighWaterMark(t *testing.T) {
	...
	t.Setenv("CONTEXT_HISTORY_BUDGET_LOW_BYTES", "4000")
	t.Setenv("CONTEXT_KEEP_RECENT_RESULTS", "1")
	// Elision spills by default, so point it somewhere disposable rather than
	// letting the test write to the real default directory.
	t.Setenv("CONTEXT_ELISION_SPILL_DIR", tempSpillDir(t))
...
func TestRunAgentLoop_ElidedOutputRecoverableViaReadFile(t *testing.T) {
	...
	// under this run's own subdirectory, not the configured dir itself.
	spilled := dirSpiller{Dir: filepath.Join(spillDir, "run-xyz")}.Path(0)
