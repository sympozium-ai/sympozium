...
| `CONTEXT_ELISION_SPILL_DIR` | `/tmp/sympozium/elided` | Where an elided result's full output is written so the model can read it back. Each run gets its own `$AGENT_RUN_ID` subdirectory under it. Set to `off` to elide destructively. Only applies when elision is on. |
...
[earlier execute_command result elided — 48213 bytes saved to
 /tmp/sympozium/elided/my-run/0007.txt; read_file to recover.]
```
...
### Where the spill lives

The default is `/tmp`, which in a run pod is an emptyDir: the spill is scratch
for one conversation and is gone when the pod is. The directory is scoped by
`$AGENT_RUN_ID` on top of that, because the filenames are ledger positions and
every run counts from `0000.txt` — without the run in the path, two runs sharing
a directory would overwrite each other.

That scoping is what makes `/workspace` a usable alternative rather than a trap.
Point `CONTEXT_ELISION_SPILL_DIR` there when you want postRun hooks to read the
spill, or when a run elides more than `/tmp`'s 256 MiB emptyDir allows:

```yaml
env:
  - name: CONTEXT_ELISION_SPILL_DIR
    value: /workspace/.sympozium/elided
```

Note that a WorkspaceSession's `/workspace` is a PVC shared by every run in the
session, so spill files put there outlive the run that wrote them — they are
per-run directories, but nothing prunes them.
...
The same fallback applies per result if a write fails at runtime (a full or
read-only volume) — logged once, then a bare placeholder. Elision still
