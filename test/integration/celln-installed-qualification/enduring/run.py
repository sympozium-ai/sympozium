...
REVISION = 'r-de5d0b56b9d0-71efec768d3c'
...
def verify_stats(stats):
    ...
    for i, e in enumerate(stats['events']):
        turn = i // 2 + 1
        check(e['accepted'] and e['turn'] == turn and e['historyExchanges'] == turn - 1 and e['stage'] == ('tool-call' if i % 2 == 0 else 'answer'), 'provider history/sequence mismatch')
...
class Qualification:
    def __init__(self, args):
        ...
        self.report = {'installedAcceptance': False, 'A03': False, 'fixture': 'deterministic provider, not real LLM', 'epoch': self.epoch, 'clusterUID': self.cluster['kubeSystemUID'], 'providerImage': args.image, 'tenants': [], 'complete': False, 'workspaceBlocker': 'workspace=none qualification only; no workspace retention claim'}
        self.created = []
        self.originals = []
        self.save()
    ...
    def setup(self):
        ...
        new['runtimeProfiles'] = [x for x in new['runtimeProfiles'] if 'enduring' in x['ref']['name']]
        ...
        for t in ('a', 'b'):
            ...
            self.create(self.obj('AgentRuntime', self.name, {'image': '', 'cellnProfileRef': {'name': 'celln-json-enduring-review-495', 'revision': REVISION}}, ns))
    ...
    def run_tenant(self, t):
        ...
        spec = {'agentRef': self.name, 'agentId': 'default', 'sessionKey': self.name, 'task': messages[0], 'systemPrompt': 'Use the selected uppercase tool exactly once on every turn and return only its text field.', 'model': {'connectionRef': self.name, 'model': 'review-uppercase'}, 'backend': 'celln', 'executionLifecycle': 'enduring', 'enduring': {'leaseSeconds': 600, 'maxTurns': 3, 'maxModelRequests': 6, 'maxOutputTokens': 3072}, 'timeout': '2m', 'cleanup': 'delete', 'cellnSelection': {'runtimeRef': self.name, 'toolRefs': [], 'clusterToolRefs': [{'name': 'uppercase-review-495', 'revision': REVISION}]}}
        ...
        for i in range(3):
            ...
            check(scoped.get('output', '').strip() == messages[i].upper(), 'wrong exact output')
...
def main():
    ...
    try:
        ...
        q.save()
        print('PASS: A/B each three turns, exact provider history, six independent attempts, closed ledgers and root teardown. installedAcceptance=false A03=false')
