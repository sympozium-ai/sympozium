...
// gatedPostRunRun builds a run parked in PostRunning behind a gate hook, which
// is the only state reconcilePostRunning is ever reached from for a gated run.
func gatedPostRunRun(name string) *sympoziumv1alpha1.AgentRun {
	exit := int32(0)
...
func postRunFixture(
	...
	r := newAgentRunTestReconciler(t, run, job)
	r.EventBus = &recordingEventBus{}
	return r, run
}

func getPostRunRun(t *testing.T, r *AgentRunReconciler, name string) *sympoziumv1alpha1.AgentRun {
	t.Helper()
...
func TestReconcilePostRunning_LongRunStillGetsItsFullGateBudget(t *testing.T) {
	...
	got := getPostRunRun(t, r, "demo-run")
...
func TestReconcilePostRunning_StaleGateJobTimesOut(t *testing.T) {
	...
	if got := getPostRunRun(t, r, "demo-run"); got.Status.GateVerdict != "error" {
...
func TestReconcilePostRunning_DeclaredHookTimeoutExtendsTheBudget(t *testing.T) {
	...
	if got := getPostRunRun(t, r, "demo-run"); got.Status.GateVerdict != "" {
